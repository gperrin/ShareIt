...
public class SharerController extends GenericController{
	...
	@RequestMapping ( method = RequestMethod.POST, value = "/user/{id:[\\d]+}/picture")
	public void uploadProfilePic(@PathVariable("id") int id, @RequestParam(value="file", required = true) MultipartFile file){
		Sharer sharer = this.sharerService.getUser(id);
		if ( sharer == null ) throw new ResourceNotFoundException("User", id);
		try {
			sharer.setProfilePicture(file.getBytes());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	@RequestMapping ( method = RequestMethod.GET, value = "/user/{id:[\\d]+}/picture")
	public void downloadProfilePic ( @PathVariable ("id")int id, HttpServletResponse response){
		Sharer sharer = this.sharerService.getUser(id);
		try {
			response.getOutputStream().write(sharer.getProfilePicture());
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		response.setContentType(sharer.getProFilePictureType().toString());
